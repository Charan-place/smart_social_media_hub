...
export default function Analytics() {
  ...
  const { data: topPostsData } = useQuery({
    queryKey: ['top-posts', 'views'],
    queryFn: () => analyticsApi.getTopPosts('views', 20),
  });

  const { data: postsData } = useQuery({
    queryKey: ['posts-all'],
    queryFn: () => postsApi.getAll({ limit: 50 }),
  });
  ...
  return (
    <div className="flex flex-col flex-1">
      <Navbar title="Analytics" subtitle="Detailed performance across all channels" />

      <div className="flex-1 p-6 space-y-6">
        {/* Filters */}
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex gap-1 bg-[#1a1a1a] p-1 rounded-lg border border-[#2f2f2f]">
            {ranges.map(r => (
              <button key={r.value} onClick={() => setRange(r.value)}
                className={`px-3 py-1.5 text-xs rounded-md transition-colors font-medium ${range === r.value ? 'bg-red-600 text-white' : 'text-gray-400 hover:text-white'}`}>
                {r.label}
              </button>
            ))}
          </div>
          <div className="flex gap-1 bg-[#1a1a1a] p-1 rounded-lg border border-[#2f2f2f]">
            {[{ v: 'all', l: 'All Platforms' }, { v: 'youtube', l: '▶ YouTube' }, { v: 'instagram', l: '◉ Instagram' }].map(p => (
              <button key={p.v} onClick={() => setPlatform(p.v as any)}
                className={`px-3 py-1.5 text-xs rounded-md transition-colors font-medium ${platform === p.v ? 'bg-red-600 text-white' : 'text-gray-400 hover:text-white'}`}>
                {p.l}
              </button>
            ))}
          </div>
          <button onClick={exportCsv} className="btn-secondary text-xs flex items-center gap-1.5 ml-auto">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={s.label} className="bg-[#0f0f0f] rounded-lg p-3 text-center">
                  ...
                  <p className="text-lg font-bold text-white">{formatNumber(s.value)}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Chart */}
        <div className="card">
          <h2 className="font-semibold text-white mb-4">Performance Over Time</h2>
          <TrendChart data={chartData} />
        </div>

        {/* Tabs */}
        <div>
          <div className="flex border-b border-[#2f2f2f] mb-4">
            {[
          ...
          {activeTab === 'channels' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(channelsData?.channels || []).map((ch: any) => (
                <div key={ch._id} className="card">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-9 h-9 rounded-full bg-[#2f2f2f] overflow-hidden flex items-center justify-center">
              ...
              {!channelsData?.channels?.length && (
                <div className="col-span-3 text-center py-8 text-gray-500 text-sm">No channels connected yet.</div>
              )}
            </div>
          )}
        </div>
